<template>
  <div class="container">
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>品相出库管理</el-breadcrumb-item>
    </el-breadcrumb>
    <el-card>
      <el-row>
        <el-button type="primary" plain size="medium" @click="showAddStockOut">新增出库单</el-button>
      </el-row>
      <el-table
...
export default {
  ...
  methods: {
    async getStockOutData(page, rows) {
      ...
      const { data: res } = await this.$http.get(`stockout/list?${params}`);
    ...
    async removeStockOut(id) {
      ...
      params = this.$qs.stringify(params);
      const res = await this.$http.delete("stockout?" + params);
      if (res.data.code === 1) {
        this.$message({
          message: "删除出库单成功",
          type: "success",
    ...
    addStockOut() {
      this.$refs.stockOutRef.validate(async (valid) => {
        ...
        const { data: res } = await this.$http.post("stockout", stockOutForm);
        if (res.code === 1) {
          this.$message({
            message: "添加成功",
            type: "success",
    ...
    async checkOrder(id) {
      ...
      params = this.$qs.stringify(params);
      const { data: res } = await this.$http.put("stockout/check?" + params);
      //  console.log(res)
      if (res.code === 1) {
        this.$message({
          message: "审核成功",
    ...
    async uncheckOrder(id) {
      let params = { id };
      params = this.$qs.stringify(params);
      const { data: res } = await this.$http.put("stockout/uncheck?" + params);
      if (res.code === 1) {
        this.$message({
          message: "反审核成功",
          type: "success",
    ...
    goDetail(id) {
      this.$router.push("/stockout/detail/" + id);
    },
    editOrderVisible(id) {
    ...
    async editStockOut() {
      ...
      params = this.$qs.stringify(params);
      const { data: res } = await this.$http.post("stockout/update", params);
      if (res.code === 1) {
        this.$message({
          message: "修改订单成功",
          type: "success",
